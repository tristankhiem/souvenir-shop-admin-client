import {AfterViewInit, Component, ElementRef, EventEmitter, Output, ViewChild} from '@angular/core';
import {AppAlert, AppLoading} from '../../../utils';
import {AppCommon} from '../../../utils/app-common';
import {WardService} from '../../../services/district/ward.service';
import {ModalWrapperComponent} from '../../commons/modal-wrapper/modal-wrapper.component';
import {NgForm} from '@angular/forms';
import {WardModel} from '../../../data-services/ward.model';
import {EmployeeModel} from '../../../data-services/employee-model';
import {ResponseModel} from '../../../data-services/response.model';
import {HTTP_CODE_CONSTANT} from '../../../constants/http-code.constant';
import {ZoningBaseFullModel} from '../../../data-services/zoning-base-full.model';
import {BaseCoordinateModel} from '../../../data-services/base-coordinate.model';
import {ZoningBaseService} from '../../../services/district/zoning-base.service';
import {InformationCoordinateModel} from '../../../data-services/information-coordinate.model';
import {findIndex} from 'rxjs/operators';
import {coordinateExtensions} from '../../../constants/accept-files.constant';

declare var $: any;
declare var readXlsxFile: any;

@Component({
  selector: 'app-add-zoning-base',
  templateUrl: './add-zoning-base.component.html'
})
export class AddZoningBaseComponent implements AfterViewInit {
  constructor(
    private loading: AppLoading,
    private alert: AppAlert,
    private common: AppCommon,
    private wardService: WardService,
    private zoningBaseService: ZoningBaseService
  ) {
  }

  @Output() saveCompleted = new EventEmitter<any>();
  @ViewChild('addZoningBaseModalWrapper', {static: true}) addZoningBaseModalWrapper: ModalWrapperComponent;
  @ViewChild('addZoningBaseForm', {static: true}) addZoningBaseForm: NgForm;
  @ViewChild('uploadCoordinateAddInput', {read: ElementRef}) uploadCoordinateAddInput: ElementRef;

  public wards: WardModel[] = [];
  public newZoningBase: ZoningBaseFullModel = new ZoningBaseFullModel();

  private isLoaded = false;
  private targetModalLoading: ElementRef;

  ngAfterViewInit(): void {
    this.targetModalLoading = $(`#${this.addZoningBaseModalWrapper.id} .modal-dialog`);
  }

  public onInit(): void {
    this.newZoningBase.ward.id = 0;

    const arrCoordinate = ['', '', '', ''].map((coor, index) => {
      const tempCoor = new BaseCoordinateModel();
      tempCoor.id = index;
      return tempCoor;
    });
    this.newZoningBase.baseCoordinates = arrCoordinate;
  }

  public show(zoningTypeId: number): void {
    this.newZoningBase.zoningType.id = zoningTypeId;

    this.onInit();
    this.addZoningBaseModalWrapper.show();

    if (!this.isLoaded) {
      this.loading.show();
      this.getWards();
    }
  }

  public hide(): void {
    this.addZoningBaseModalWrapper.hide();
  }

  public onHideEvent(): void {
    this.newZoningBase = new ZoningBaseFullModel();
    this.addZoningBaseForm.onReset();
  }

  public addCoordinate(): void {
    const tempCoordinate = new BaseCoordinateModel();

    const sizeCoors = this.newZoningBase.baseCoordinates.length;
    tempCoordinate.id = sizeCoors ? this.newZoningBase.baseCoordinates[sizeCoors - 1].id + 1 : 0;

    this.newZoningBase.baseCoordinates.push(tempCoordinate);
  }

  public removeCoordinate(tempId: number): void {
    const indexCoor = this.newZoningBase.baseCoordinates.findIndex( coor => +coor.id === +tempId );
    this.newZoningBase.baseCoordinates.splice(indexCoor, 1);
  }

  public uploadCoordinate(event: Event): void {
    const uploadInput: HTMLInputElement = event.target as HTMLInputElement;

    if (!uploadInput.files[0]) {
      return;
    }

    const fileName = uploadInput.files[0].name.split('.');
    const isValidExtension = coordinateExtensions.findIndex( ext => ext === fileName[fileName.length - 1] );

    if (isValidExtension === -1) {
      this.alert.error('Tệp tin không hợp lệ');
      return;
    }

    const arrCoordinates = [];
    readXlsxFile(uploadInput.files[0]).then((rows) => {
      for (const row of rows) {
        if (isNaN(+row[0]) || isNaN(+row[1])) {
          this.alert.error('Tệp tin không hợp lệ');
          return;
        }
        arrCoordinates.push(row);
      }

      this.newZoningBase.baseCoordinates = arrCoordinates.map((coor, index) => {
        const tempCoor = new BaseCoordinateModel();
        tempCoor.id = index;
        tempCoor.xcoordinate = coor[0];
        tempCoor.ycoordinate = coor[1];

        return tempCoor;
      });

      this.alert.success('Tải thành công');
      this.uploadCoordinateAddInput.nativeElement.value = '';
    });
  }

  public isValid(): boolean {
    if (this.addZoningBaseForm.invalid) {
      return false;
    }

    if (+this.newZoningBase.ward.id === 0) {
      return false;
    }

    const lengthCoors = this.newZoningBase.baseCoordinates.length;
    if (lengthCoors < 4) {
      return false;
    }

    const firstCoor = this.newZoningBase.baseCoordinates[0];
    const lastCoor = this.newZoningBase.baseCoordinates[lengthCoors - 1];
    if (firstCoor.xcoordinate !== lastCoor.xcoordinate || firstCoor.ycoordinate !== lastCoor.ycoordinate) {
      return false;
    }

    return true;
  }

  public onSave(): void {
    if (!this.isValid()) {
      return;
    }

    this.saveZoningBase();

  }

  private getWards(): void {
    this.wardService.findAll().subscribe(res => this.getWardsCompleted(res));
  }

  private getWardsCompleted(res: ResponseModel<WardModel[]>): void {
    this.loading.hide();
    if (res.status !== HTTP_CODE_CONSTANT.OK) {
      this.alert.errorMessages(res.message);
      return;
    }

    this.wards = res.result;
    this.isLoaded = true;
  }

  private saveZoningBase(): void {
    this.loading.show(this.targetModalLoading);
    this.zoningBaseService.save(this.newZoningBase).subscribe(res => this.saveZoningBaseCompleted(res));
  }

  private saveZoningBaseCompleted(res: ResponseModel<EmployeeModel>): void {
    this.loading.hide(this.targetModalLoading);
    if (res.status !== HTTP_CODE_CONSTANT.OK) {
      this.alert.errorMessages(res.message);
      return;
    }

    this.alert.successMessages(res.message);
    this.saveCompleted.emit();
    this.hide();
  }
}
